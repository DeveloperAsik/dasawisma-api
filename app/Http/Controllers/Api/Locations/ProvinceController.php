...
class ProvinceController extends Controller {
    ...
    public function get_list() {
        ...
        if (isset($user_token) && !empty($user_token)) {
            ...
            $res = $Tbl_a_provinces->find('all', array('fields' => 'all', 'table_name' => 'tbl_a_provinces', 'conditions' => array('where' => array('a.is_active' => '="1"'))));
        ...
        if (isset($user_token) && !empty($user_token)) {
            ...
            if (isset($post) && !empty($post)) {
               $id = base64_decode($post['id']);
                $Tbl_a_provinces = new Tbl_a_provinces();
                $res = $Tbl_a_provinces->find('all', array('fields' => 'all', 'table_name' => 'tbl_a_provinces', 'conditions' => array('where' => array('is_active' => '="1"', 'a.id' => '="' . $id . '"'))));
                if (isset($res) && !empty($res) && $res != null) {
                    return json_encode(array('status' => 200, 'message' => 'Successfully retrieving data.', 'data' => $res));
                } else {
